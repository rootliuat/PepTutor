...
set "BACKEND_PUBLIC_HOST=127.0.0.1"
...
if not "%PEPTUTOR_LESSON_BACKEND_PUBLIC_HOST%"=="" set "BACKEND_PUBLIC_HOST=%PEPTUTOR_LESSON_BACKEND_PUBLIC_HOST%"
...
set "BACKEND_URL=http://%BACKEND_PUBLIC_HOST%:%BACKEND_PORT%"
set "BACKEND_WAIT_URL=http://127.0.0.1:%BACKEND_PORT%"
...
echo Starting PepTutor lesson backend: bind=%BACKEND_HOST%:%BACKEND_PORT% url=%BACKEND_URL%
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$deadline=(Get-Date).AddSeconds(120); do { try { $r=Invoke-WebRequest -UseBasicParsing '%BACKEND_WAIT_URL%/lesson/catalog' -TimeoutSec 5; if ($r.StatusCode -eq 200) { exit 0 } } catch { }; Start-Sleep -Seconds 1 } while ((Get-Date) -lt $deadline); exit 1"
if errorlevel 1 (
  echo Timed out waiting for lesson backend: %BACKEND_WAIT_URL%/lesson/catalog
  exit /b 1
)
...
start "PepTutor Lesson Frontend" /D "%FRONTEND_DIR%" cmd /k "pnpm -F @proj-airi/stage-web exec vite --host 0.0.0.0 --port %FRONTEND_PORT% --strictPort"
