...
// This store is local-only UI state. Lesson turns may mirror transcript/debug data here,
// but prompt-time learner memory stays on the backend.
...
export const useCharacterNotebookStore = defineStore('character-notebook', () => {
  const entries = useLocalStorageManualReset<NotebookEntry[]>('character/notebook/entries', [])
  const tasks = useLocalStorageManualReset<ScheduledTask[]>('character/notebook/tasks', [])

  // `diary` is the legacy persisted kind. In lesson mode it represents a local transcript/debug mirror.
  const transcriptEntries = computed(() => entries.value.filter(entry => entry.kind === 'diary'))
  const debugEntries = computed(() => entries.value.filter(entry => entry.kind === 'note'))
  const partitionDiary = transcriptEntries
  ...
  function addDebugEntry(text: string, options?: { tags?: string[], metadata?: Record<string, unknown> }) {
    return addEntry('note', text, options)
  }

  function addNote(text: string, options?: { tags?: string[], metadata?: Record<string, unknown> }) {
    return addDebugEntry(text, options)
  }

  function addTranscriptEntry(text: string, options?: { tags?: string[], metadata?: Record<string, unknown> }) {
    return addEntry('diary', text, options)
  }

  function addDiaryEntry(text: string, options?: { tags?: string[], metadata?: Record<string, unknown> }) {
    return addTranscriptEntry(text, options)
  }
