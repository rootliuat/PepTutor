...
import { createPepTutorEdgeSpeechProvider, EDGE_TTS_DEFAULT_MODEL, EDGE_TTS_DEFAULT_VOICE } from './providers/edge/audio-speech'
...
export const useProvidersStore = defineStore('providers', () => {
  ...
  const providerMetadata: Record<string, ProviderMetadata> = {
    ...
    'peptutor-edge-tts': {
      id: 'peptutor-edge-tts',
      category: 'speech',
      tasks: ['text-to-speech', 'tts'],
      nameKey: 'settings.pages.providers.provider.peptutor-edge-tts.title',
      name: 'PepTutor Edge TTS',
      descriptionKey: 'settings.pages.providers.provider.peptutor-edge-tts.description',
      description: 'PepTutor backend proxy for Microsoft Edge Xiaoxiao voice.',
      iconColor: 'i-lobe-icons:microsoft',
      defaultOptions: () => ({
        proxyUrl: '/api/peptutor/edge-tts',
      }),
      createProvider: async config => createPepTutorEdgeSpeechProvider(config),
      capabilities: {
        listVoices: async () => [
          {
            id: EDGE_TTS_DEFAULT_VOICE,
        ...
        listModels: async () => [
          {
            id: EDGE_TTS_DEFAULT_MODEL,
            name: EDGE_TTS_DEFAULT_MODEL,
            provider: 'peptutor-edge-tts',
            description: 'Microsoft Edge TTS backend proxy',
            contextLength: 0,
            deprecated: false,
          },
        ],
      ...
      validators: {
        validateProviderConfig: (config) => {
          const proxyUrl = typeof config.proxyUrl === 'string' ? config.proxyUrl.trim() : ''
          const errors = [
            !proxyUrl && new Error('Proxy URL is required.'),
          ].filter(Boolean)
    ...
    'volcengine': {
      ...
      validators: {
        validateProviderConfig: (config) => {
          const proxyUrl = typeof config.proxyUrl === 'string' ? config.proxyUrl.trim() : ''
          const cluster = typeof (config.app as any)?.cluster === 'string' ? ((config.app as any).cluster as string).trim() : ''
          const errors = [
            !proxyUrl && new Error('Proxy URL is required.'),
            !cluster && new Error('Cluster is required.'),
          ].filter(Boolean)

          return {
            errors,
            reason: errors.filter(e => e).map(e => String(e)).join(', ') || '',
            valid: !!proxyUrl && !!cluster,
          }
        },
      },
    },
  ...
  function hasCredentialLikeConfigValue(value: unknown, keyPath: string[] = []): boolean {
    if (typeof value === 'string') {
      const key = keyPath.at(-1)?.toLowerCase() ?? ''
      return value.trim().length > 0 && /api.*key|access.*key|secret|token|credential|appkey|appid/.test(key)
    }

    if (Array.isArray(value)) {
      return value.some(item => hasCredentialLikeConfigValue(item, keyPath))
    }

    if (value && typeof value === 'object') {
      return Object.entries(value as Record<string, unknown>).some(([key, nested]) =>
        hasCredentialLikeConfigValue(nested, [...keyPath, key]),
      )
    }
  ...
  function shouldAutoValidateProvider(providerId: string) {
    if (providerId === 'speech-noop' || providerId === 'browser-web-speech-api') {
      return true
    }
    ...
    const config = providerCredentials.value[providerId]
    return isProviderConfigDirty(providerId) && hasCredentialLikeConfigValue(config)
  }
  ...
  function startPeriodicRuntimeValidation() {
    for (const [providerId, intervalMs] of providerValidationIntervalMsById.entries()) {
      ...
      const loop = useIntervalFn(() => {
        if (shouldAutoValidateProvider(providerId)) {
