import type { MetadataEventSource, WebSocketBaseEvent, WebSocketEvent, WebSocketEvents } from '@proj-airi/server-shared/types'
...
type RuntimeWebSocketEvent = WebSocketEvent<Record<string, unknown> | undefined>
...
function createServerResponse<T extends keyof WebSocketEvents>(
  type: T,
  data: WebSocketEvents[T],
  serverInstanceId: string,
  parentId?: string,
): WebSocketBaseEvent<T, WebSocketEvents[T]> {
  return {
    type,
    data,
...
const RESPONSES = {
  authenticated: (serverInstanceId: string, parentId?: string) => createServerResponse('module:authenticated', { authenticated: true }, serverInstanceId, parentId),
  notAuthenticated: (serverInstanceId: string, parentId?: string) => createServerResponse('error', { message: ServerErrorMessages.notAuthenticated }, serverInstanceId, parentId),
  error: (message: string, serverInstanceId: string, parentId?: string) => createServerResponse('error', { message }, serverInstanceId, parentId),
  heartbeat: (kind: MessageHeartbeatKind, message: MessageHeartbeat | string, serverInstanceId: string, parentId?: string) => createServerResponse('transport:connection:heartbeat', { kind, message, at: Date.now() }, serverInstanceId, parentId),
}

const DEFAULT_HEARTBEAT_TTL_MS = 60_000

// helper send function
function send(peer: Peer, event: RuntimeWebSocketEvent | string) {
...
export function setupApp(options?: AppOptions): { app: H3, closeAllPeers: () => void } {
  ...
  function broadcastToAuthenticated(event: RuntimeWebSocketEvent) {
    for (const p of peers.values()) {
      if (p.authenticated) {
        send(p.peer, event)
      }
  ...
  app.get('/ws', defineWebSocketHandler({
    ...
    message: (peer, message) => {
      ...
      let event: RuntimeWebSocketEvent
      ...
      try {
        ...
        const parsed = parse<RuntimeWebSocketEvent>(text)
        ...
        event = potentialEvent as RuntimeWebSocketEvent
      }
      catch (err) {
        ...
        case 'ui:configure': {
          ...
          if (target) {
            send(target.peer, {
              ...
              metadata: event.metadata ?? createServerEventMetadata(instanceId),
            })
          }
          else {
