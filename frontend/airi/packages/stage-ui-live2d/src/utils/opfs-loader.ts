...
export class OPFSCache {
  static isZipLikeSourceUrl(sourceUrl: string): boolean {
    ...
    try {
      ...
      return parsed.pathname.endsWith('.zip') || parsed.pathname.startsWith('/__airi/live2d/preset/')
    }
    catch {
      return sourceUrl.endsWith('.zip') || sourceUrl.includes('/__airi/live2d/preset/')
    }
  }
  ...
  static checkMiddleware: Middleware<OPFSContext> = async (context, next) => {
    ...
    if (!key || !blobUrl || !OPFSCache.isZipLikeSourceUrl(blobUrl)) {
