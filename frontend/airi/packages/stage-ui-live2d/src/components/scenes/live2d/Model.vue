...
declare global {
  interface Window {
    __PEPTUTOR_LIPSYNC_MODEL_DEBUG__?: {
...
const runtimeMouthGain = 1.4
const mouthOpenSize = computed(() => clampUnit((props.mouthOpenSize ?? 0) * runtimeMouthGain))
const runtimeMouthForm = computed(() => clampSignedUnit(props.mouthFormValue))
const effectiveMouthOpen = computed(() => Math.max(
  mouthOpenSize.value,
  clampUnit(modelParameters.value.mouthOpen),
))
const effectiveMouthForm = computed(() => clampSignedUnit(runtimeMouthForm.value + modelParameters.value.mouthForm))

function applyMouthParameters(target = model.value?.internalModel.coreModel) {
  if (!target)
    return

  target.setParameterValueById('ParamMouthOpenY', effectiveMouthOpen.value)
  target.setParameterValueById('ParamMouthForm', effectiveMouthForm.value)
}

function registerModelLipSyncDebugHandle() {
  if (!import.meta.env.DEV || typeof window === 'undefined')
    return

  window.__PEPTUTOR_LIPSYNC_MODEL_DEBUG__ = {
    state: () => ({
      runtimeMouthOpen: mouthOpenSize.value,
      runtimeMouthForm: runtimeMouthForm.value,
      effectiveMouthOpen: effectiveMouthOpen.value,
      effectiveMouthForm: effectiveMouthForm.value,
      paramMouthOpenY: model.value?.internalModel.coreModel.getParameterValueById('ParamMouthOpenY') ?? null,
      paramMouthForm: model.value?.internalModel.coreModel.getParameterValueById('ParamMouthForm') ?? null,
    }),
  }
}

function clearModelLipSyncDebugHandle() {
  if (!import.meta.env.DEV || typeof window === 'undefined')
    return

  delete window.__PEPTUTOR_LIPSYNC_MODEL_DEBUG__
}
...
async function loadModel() {
  ...
  try {
    ...
    applyMouthParameters(coreModel)
    ...
    motionManagerUpdate.register((ctx) => {
      applyMouthParameters(ctx.model)
    }, 'post')
...
watch([effectiveMouthOpen, effectiveMouthForm], () => applyMouthParameters())
...
watch(() => modelParameters.value.mouthOpen, () => {
  if (model.value) {
    applyMouthParameters(model.value.internalModel.coreModel)
  }
})
...
onMounted(() => {
  registerModelLipSyncDebugHandle()
