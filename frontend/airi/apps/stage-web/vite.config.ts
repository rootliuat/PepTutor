...
const frontendRoot = resolve(join(import.meta.dirname, '..', '..', '..'))
...
function resolveDevHttpsConfig(mergedEnv: Record<string, string | undefined>) {
  const keyPath = mergedEnv.VITE_PEPTUTOR_DEV_HTTPS_KEY?.trim()
  const certPath = mergedEnv.VITE_PEPTUTOR_DEV_HTTPS_CERT?.trim()
  if (!keyPath || !certPath) {
    return undefined
  }
...
function resolvePepTutorDevProxyConfig(mergedEnv: Record<string, string | undefined>) {
  const target = (mergedEnv.VITE_PEPTUTOR_DEV_PROXY_TARGET || 'http://127.0.0.1:9625').trim()
  ...
  return {
    '/peptutor-api': {
      ...
      secure: false,
      ws: true,
      rewrite: (path: string) => path.replace(/^\/peptutor-api/, ''),
    },
  }
}
...
export default defineConfig(({ mode }) => {
  ...
  const enableVueDevTools = ['1', 'true', 'yes'].includes(
    (mergedEnv.VITE_AIRI_ENABLE_VUE_DEVTOOLS || '').toLowerCase(),
  )
  ...
  return {
    ...
    server: {
      https: resolveDevHttpsConfig(mergedEnv),
      proxy: resolvePepTutorDevProxyConfig(mergedEnv),
      fs: {
        // To mute errors like:
