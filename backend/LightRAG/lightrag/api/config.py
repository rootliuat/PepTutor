...
REMOTE_ONLY_LLM_BINDINGS = frozenset({"lollms", "ollama", "openai-ollama"})
REMOTE_ONLY_EMBEDDING_BINDINGS = frozenset({"lollms", "ollama"})
REMOTE_HOST_REQUIRED_LLM_BINDINGS = frozenset({"openai", "azure_openai", "gemini"})
REMOTE_HOST_REQUIRED_EMBEDDING_BINDINGS = frozenset(
    {"openai", "azure_openai", "gemini", "jina"}
)
REMOTE_API_KEY_REQUIRED_LLM_BINDINGS = frozenset(
    {"openai", "azure_openai", "gemini"}
)
REMOTE_API_KEY_REQUIRED_EMBEDDING_BINDINGS = frozenset(
    {"openai", "azure_openai", "gemini", "jina"}
)
OBVIOUSLY_LOCAL_HOSTNAMES = frozenset(
    {"localhost", "127.0.0.1", "0.0.0.0", "::1", "host.docker.internal"}
)
DEFAULT_LOCAL_LLM_MODEL = "mistral-nemo:latest"
...
def _normalize_hostname(binding_host: str | None) -> str | None:
    if not binding_host:
        return None

    candidate = binding_host.strip()
    if not candidate:
        return None
...
def _is_obviously_local_host(binding_host: str | None) -> bool:
    hostname = _normalize_hostname(binding_host)
    if not hostname:
        return False
    return hostname in OBVIOUSLY_LOCAL_HOSTNAMES


def _resolve_llm_host(args: argparse.Namespace) -> str | None:
    if getattr(args, "llm_binding", None) == "aws_bedrock":
        return None
    return getattr(args, "llm_binding_host", None)


def _resolve_embedding_host(args: argparse.Namespace) -> str | None:
    binding = getattr(args, "embedding_binding", None)

    if binding == "aws_bedrock":
        return None
    if binding == "azure_openai":
        return os.getenv("AZURE_EMBEDDING_ENDPOINT") or getattr(
            args, "embedding_binding_host", None
        )
    if binding == "jina":
        return getattr(args, "embedding_binding_host", None) or os.getenv(
            "JINA_API_BASE", "https://api.jina.ai/v1/embeddings"
        )
    return getattr(args, "embedding_binding_host", None)


def _has_any_env_value(*names: str) -> bool:
    return any(os.getenv(name) for name in names)


def enforce_remote_model_configuration(args: argparse.Namespace) -> argparse.Namespace:
    args.peptutor_require_remote_models = get_env_value(
        "PEPTUTOR_REQUIRE_REMOTE_MODELS", False, bool
    )

    if not args.peptutor_require_remote_models:
        return args

    problems: list[str] = []
    llm_binding = getattr(args, "llm_binding", None)
    embedding_binding = getattr(args, "embedding_binding", None)

    if llm_binding in REMOTE_ONLY_LLM_BINDINGS:
        problems.append(
            f"LLM_BINDING={llm_binding} is a local-model binding. Use a hosted API binding instead."
        )

    if embedding_binding in REMOTE_ONLY_EMBEDDING_BINDINGS:
        problems.append(
            "EMBEDDING_BINDING="
            f"{embedding_binding} is a local-model binding. Use a hosted API binding instead."
        )

    llm_host = _resolve_llm_host(args)
    if llm_binding in REMOTE_HOST_REQUIRED_LLM_BINDINGS and _is_obviously_local_host(
        llm_host
    ):
        problems.append(
            f"LLM_BINDING_HOST resolves to a local endpoint ({llm_host}). "
            "Point it at a hosted API endpoint."
        )

    embedding_host = _resolve_embedding_host(args)
    if (
        embedding_binding in REMOTE_HOST_REQUIRED_EMBEDDING_BINDINGS
        and _is_obviously_local_host(embedding_host)
    ):
        problems.append(
            "EMBEDDING_BINDING_HOST resolves to a local endpoint "
            f"({embedding_host}). Point it at a hosted API endpoint."
        )

    if (
        llm_binding in REMOTE_API_KEY_REQUIRED_LLM_BINDINGS
        and not getattr(args, "llm_binding_api_key", None)
        and not _has_any_env_value("LLM_BINDING_API_KEY", "AZURE_OPENAI_API_KEY", "GEMINI_API_KEY")
    ):
        problems.append(
            f"LLM binding {llm_binding} requires remote credentials. "
            "Set LLM_BINDING_API_KEY or the provider-specific API key."
        )

    if (
        embedding_binding in REMOTE_API_KEY_REQUIRED_EMBEDDING_BINDINGS
        and not getattr(args, "embedding_binding_api_key", None)
        and not _has_any_env_value(
        ...
        problems.append(
            f"Embedding binding {embedding_binding} requires remote credentials. "
            "Set EMBEDDING_BINDING_API_KEY or the provider-specific API key."
        )

    if getattr(args, "llm_model", DEFAULT_LOCAL_LLM_MODEL) == DEFAULT_LOCAL_LLM_MODEL:
        problems.append(
            "LLM_MODEL is still the local default `mistral-nemo:latest`. "
            "Set it to the hosted model or deployment name you actually use."
        )

    if problems:
        bullet_list = "\n".join(f"- {problem}" for problem in problems)
        raise ValueError(
            "PEPTUTOR_REQUIRE_REMOTE_MODELS=1 rejected the current model configuration:\n"
            f"{bullet_list}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    # PepTutor route rate limiting (per-process fixed window)
    args.peptutor_lesson_rate_limit_requests = get_env_value(
        "PEPTUTOR_LESSON_RATE_LIMIT_REQUESTS", 60, int
    )
    args.peptutor_lesson_rate_limit_window_seconds = get_env_value(
        "PEPTUTOR_LESSON_RATE_LIMIT_WINDOW_SECONDS", 60, int
    )
    args.peptutor_speech_tts_rate_limit_requests = get_env_value(
        "PEPTUTOR_SPEECH_TTS_RATE_LIMIT_REQUESTS", 20, int
    )
    args.peptutor_speech_tts_rate_limit_window_seconds = get_env_value(
        "PEPTUTOR_SPEECH_TTS_RATE_LIMIT_WINDOW_SECONDS", 60, int
    )
    args.peptutor_speech_asr_connect_rate_limit_requests = get_env_value(
        "PEPTUTOR_SPEECH_ASR_CONNECT_RATE_LIMIT_REQUESTS", 6, int
    )
    args.peptutor_speech_asr_connect_rate_limit_window_seconds = get_env_value(
        "PEPTUTOR_SPEECH_ASR_CONNECT_RATE_LIMIT_WINDOW_SECONDS", 60, int
    )
    ...
    enforce_remote_model_configuration(args)
    return args
...
def initialize_config(args=None, force=False):
    ...
    _global_args = enforce_remote_model_configuration(resolved_args)
    _initialized = True
    return _global_args
