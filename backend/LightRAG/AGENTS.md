...
- For PepTutor `/lesson` real-browser smoke from `frontend/airi`, prefer `bash ../scripts/smoke_lesson_browser.sh` from the repo root. That helper starts `./.venv/bin/lightrag-server --host 127.0.0.1 --port 9625` with the route-focused lesson env, waits for readiness, runs the checked-in browser suite, and cleans up automatically. Use a dedicated long-lived server shell only for interactive debugging. `scripts/smoke_lesson_turn.py` remains the direct route-contract smoke and is not the default browser-smoke entry point.
